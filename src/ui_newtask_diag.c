...
void ui_newtask()
{
	...
	w_prj = GTK_COMBO_BOX_TEXT(gtk_builder_get_object(builder, "diag_tasknew_project"));
	...
	if (result == GTK_RESPONSE_ACCEPT)
	{
		...
		entry = GTK_ENTRY(gtk_builder_get_object(builder, "diag_tasknew_description"));
		desc = gtk_entry_get_text(entry);

		w_prj = GTK_COMBO_BOX_TEXT(gtk_builder_get_object(builder, "diag_tasknew_project"));
		prj = gtk_combo_box_text_get_active_text(w_prj);

		combo = GTK_COMBO_BOX(gtk_builder_get_object(builder, "diag_tasknew_priority"));
